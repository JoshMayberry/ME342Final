...
#{way1: [[eqnTitles],[[varAvailable],[varToSolveFor]],[eqnsLeft]],
...
for k in range(len(unknownList)-1):#This loop will end when just the goal var remains
    ...
    for item in temp.items(): #Look at each way
        ...
        print('available', unknownNeedList)
        print('new',unknownHaveList) #Variables to solve for
        ...
        for i in range(len(tempMatrix[1])):
            ...
            if (sum(tempMatrix[1][i][0:-1]) == 1) and (tempMatrix[1][i][-1] != 1):  
                ...
                if passed == True: #If there is another way
                    ...
                    tempNew.update({item[0]:[tempMatrix[0],[unknownNeedListTemp,unknownListTemp],tempMatrix[1]]}) #If this is the first way of the many or just the only way.
                passed = True
