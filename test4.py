...
nextUnknownList = [[],copy.deepcopy(unknownList)]
nextUnknownListTemp = [[],[]]
for k in range(len(unknownList)-1):#This loop will end when just the goal var remains
    ...
    for item in temp.items(): #Look at each way
        ...
        print('next',nextUnknownList) #Variables available
        print('new',newUnknownList) #Variables to solve for
        for var in nextUnknownList[1]: #Lets remove the variable that is solved in this way.
            if k != 0:
                    compare = copy.deepcopy(unknownList)
                    compare.remove(
                    print(stop)
                    #for element in tempMatrixTemp
                    #tempMatrixTemp.remove(nextUnknownList[1][m]
                    #if item[0] == nextUnknownList[0]
            else: compare = newUnknownList
            if var not in compare:
                n = unknownList.index(var)
                for row in tempMatrix[1]: del row[n]
                nextUnknownListTemp[0].append(item[0])
                nextUnknownListTemp[1].append(var)
#            print(nextUnknownListTemp)  
    ...
    nextUnknownList = copy.deepcopy(nextUnknownListTemp)
