...
for i in range(len(myMatrix[1])):
    if (sum(myMatrix[1][i][0:-1]) == 1) and (myMatrix[1][i][-1] != 1):
        ...
        temp.update({'way'+str(wayCount):[[myMatrix[0][i]],[unknownListTemp],[]]}) #{way1: [[eqnTitles],[varToSolveFor],[eqnsLeft]],
...
for item in temp.items(): #Look at each way
    newUnknownList = copy.deepcopy(item[1][1][0])
    tempMatrix = copy.deepcopy(item[1][2])
    for var in unknownList: #Lets remove the variable that is solved in this way.
        if var not in newUnknownList:
            n = unknownList.index(var)
            if n != 'used':
                for row in tempMatrix[1]: del row[n]
    second,i = False,0 #For if there is another way that branches off of this way.
    for i in range(len(tempMatrix[1])):
        if (sum(tempMatrix[1][i][0:-1]) == 1) and (tempMatrix[1][i][-1] != 1):  
            ...
            if second == True: #If there is another way
                wayCount += 1
                tempNew.update({'way'+str(wayCount):[[myMatrix[0][i]],[unknownListTemp],[]]}) #{way1: [[eqnTitles],[varToSolveFor],[eqnsLeft]],
            else: tempNew.update({item[0]:[[myMatrix[0][i]],[unknownListTemp],[]]}) #If this is the first way of the many or just the only way.
            second = True
...
print(tempNew)
#Once only the goal remains, delete any extra eqns. Now, we should have several matricies that solve the problem in every possible way. If any pathways were not able to make it to the goal, delete them.
    ##temp = {way1: [[eqnTitles],[varToSolveFor],[eqnsLeft]],way2: [[eqnTitles],[varToSolveFor],[eqnsLeft]], etc...} Just pass through the titles
myMatrix = copy.deepcopy(tempNew) #Replace temp with tempNew
